...
WALLET_NO = 1200    # No of similar wallet address to be considered
...
def UserKNN(user_array, vec, all_tokens):
    ...
    token_scores = np.array(token_scores) * 100 / max(token_scores)
    
    return {all_tokens[i]: token_scores[i] for i in range(100)}
